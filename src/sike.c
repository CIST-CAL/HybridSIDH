...
int Hy_crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{ // SIKE's key generation
  ...
  //          public key pk (CRYPTO_PUBLICKEYBYTES bytes) 
    ...
    Hy_KeyGeneration_B(sk + MSG_BYTES, pk);
...
int Hy_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{ // SIKE's encapsulation
  ...
  //          ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = CRYPTO_PUBLICKEYBYTES + MSG_BYTES bytes)
    ...
    // Encrypt
    Hy_KeyGeneration_A(ephemeralsk, ct);
    Hy_SecretAgreement_A(ephemeralsk, pk, jinvariant);
...
int Hy_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{ // SIKE's decapsulation
  ...
  // Outputs: shared secret ss      (CRYPTO_BYTES bytes)
    ...
    // Decrypt
    Hy_SecretAgreement_B(sk + MSG_BYTES, ct, jinvariant_);
    ...
    Hy_KeyGeneration_A(ephemeralsk_, c0_);
