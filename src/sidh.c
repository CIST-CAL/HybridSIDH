...
int Hy_KeyGeneration_A(const unsigned char* PrivateKeyA, unsigned char* PublicKeyA)
{ // Alice's ephemeral public key generation
  ...
  // Output: the public key PublicKeyA consisting of 3 elements in GF(p^2) which are encoded by removing leading 0 bytes.
	...
	point_proj_ED_t E_R;
	f2elm_t XPA, XQA, XRA, coeff, A24plus = { 0 }, C24 = { 0 }, A={0}, t0;
	unsigned int i, row, m, index = 0, pts_index[MAX_INT_POINTS_ALICE], npts = 0, ii = 0;
	int get_isog_cnt=0, eval_isog_cnt=0;
	...
	LADDER3PT(XPA, XQA, XRA, (digit_t*)PrivateKeyA, ALICE, R, A); // Mont
    ...
    for (row = 1; row < MAX_Alice; row++) {
        while (index < MAX_Alice-row) {
			...
			m = strat_Alice[ii++];
			xDBLe(R, R, A24plus, C24, (int)(2 * m)); // Doubling on Montogmery
			index += m;
		}

		fp2sub(R->X, R->Z, E_R->Y);
		fp2add(R->X, R->Z, E_R->Z);           // swith to edwards

		get_4_isog_ED(R, A24plus, C24, coeff);

		for (i = 0; i < npts; i++) {
			
			eval_4_isog_ED(E_R, coeff, pts[i]);

		}

		eval_4_isog_ED(E_R, coeff, phiP);
		eval_4_isog_ED(E_R,  coeff, phiQ);
		eval_4_isog_ED(E_R, coeff, phiR);
	...
	fp2sub(R->X, R->Z, E_R->Y);
	fp2add(R->X, R->Z, E_R->Z);        

	fp2add(R->X, R->X, t0);
	fp2sqr_mont(t0, coeff);							// C = (Y4+Z4)^2/4
...
int Hy_KeyGeneration_B(const unsigned char* PrivateKeyB, unsigned char* PublicKeyB)
{ // Bob's ephemeral public key generation
  ...
  // Output: the public key PublicKeyB consisting of 3 elements in GF(p^2) which are encoded by removing leading 0 bytes.
	...
	//Convert Basis R to Edwards curves
	...
	for (row = 1; row < MAX_Bob; row++) {
		while (index < MAX_Bob - row) {
			...
			pts_index[npts++] = index;
			m = strat_Bob[ii++];
			xTPLe_ED(R, R, A24plus, A24minus, (int)m);
			index += m;
		}
		...
		get_3_isog_ED(E_R, A24plus, A24minus);

		for (i = 0; i < npts; i++) {
 
			eval_3_isog_ED(E_R,  pts[i]);
		}

		eval_3_isog_ED(E_R,  phiP);
		eval_3_isog_ED(E_R, phiQ);
		eval_3_isog_ED(E_R,  phiR);
...
int Hy_SecretAgreement_A(const unsigned char* PrivateKeyA, const unsigned char* PublicKeyB, unsigned char* SharedSecretA)
{ // Alice's ephemeral shared secret computation
  ...
  // Output: a shared secret SharedSecretA that consists of one element in GF(p^2) encoded by removing leading 0 bytes.  
	...
	point_proj_ED_t E_R;
	f2elm_t coeff, PKB[3], jinv;
	f2elm_t A24plus = { 0 }, C24 = { 0 },A={0}, C = { 0 }, D = { 0 };
	unsigned int i, row, m, index = 0, pts_index[MAX_INT_POINTS_ALICE], npts = 0, ii = 0;
...
int Hy_SecretAgreement_B(const unsigned char* PrivateKeyB, const unsigned char* PublicKeyA, unsigned char* SharedSecretB)
{ // Bob's ephemeral shared secret computation
  ...
  // Output: a shared secret SharedSecretB that consists of one element in GF(p^2) encoded by removing leading 0 bytes.  
	...
	f2elm_t A24plus = { 0 }, A24minus = {0}, C24 = { 0 }, A = { 0 }, C = { 0 }, D = { 0 };
	unsigned int i, row, m, index = 0, pts_index[MAX_INT_POINTS_BOB], npts = 0, ii = 0;
...
// Mont to Ed
	...
	get_3_isog_ED(E_R, A24plus, A24minus);
    fp2add(A24plus, A24minus, C);		 // C = C+D
    fp2sub(A24plus, A24minus, D);
